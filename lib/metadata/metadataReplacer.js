...
import { isClownface, toNamedNode } from '../util/parameterUtils.js'
...
function subjectsWithDatasetType (dataset, classes) {
  ...
  dataset.filter(quad => (quad.predicate.equals(ns.rdf.type) && classes.has(quad.object))).forEach(quad => {
...
function replaceMetadata (dataset, metadata = {}, { dateModified, dateCreated, graph }) {
  // dateModified
  if (dateModified) {
    const dateModifiedLiteral = resolveNamedDate(dateModified, metadata)
  ...
  // dateCreated
  if (dateCreated) {
    const dateCreatedLiteral = resolveNamedDate(dateCreated, metadata)
  ...
  // Sets graph
  if (graph) {
    dataset = dataset.map(quad => rdf.quad(quad.subject, quad.predicate, quad.object, toNamedNode(graph)))
  }

  return dataset
}

function createMetadataReplacer ({ dateModified, dateCreated, graph }) {
  return (dataset, metadata) => replaceMetadata(dataset, metadata, { dateModified, dateCreated, graph })
}

export { createMetadataReplacer }
