...
import { localFetch } from '../localFetch/localFetch'
...
const boundedDescription = rdf.traverser(({
  dataset,
  level,
  quad
...
async function readFromLocal (input, basePath, pointer) {
  ...
  return pointer ? {
      dataset: boundedDescription.match(rdf.clownface({ dataset: dataset, term: rootTerm.term })), metadata
    }
